...
async def get_formats(CallbackQuery, videoid, user_id, type):
    ...
    try:
        ...
        return await CallbackQuery.message.reply_text(
            "Gagal Mengambil data dari YouTube...Bisa jadi masalah YTDL."
        )
    j = 0
    for x in formats:
        check = x["format"]
        if type == "audio":
            ...
            return await CallbackQuery.message.reply_text(
                "Format Video tidak ditemukan. Coba mencari video lainnya"
            )
    if j == 0:
        return await CallbackQuery.message.reply_text(
            "Format Video tidak ditemukan. Coba mencari video lainnya"
        ...
        key = InlineKeyboardMarkup(
            [
                ...
                [
                    ...
                    InlineKeyboardButton(
                        text="🗑 ᴛᴜᴛᴜᴘ", callback_data=f"close2"
                    ),
                ],
            ]
        )
        ...
        return await CallbackQuery.message.reply_text(
            "Format video tidak ditemukan. Coba mencari musik lainnya"
        )
    return key
