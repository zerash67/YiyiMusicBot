...
async def start_stream_video(message, file, title, mystic):
    ...
    if await is_active_chat(message.chat.id):
        ...
        await mystic.delete()
        cap = f"🎥<b>__Bermain:__ </b>[Video Melalui Telegram]({link})\n👤**__Atas Permintaan:__** {message.from_user.mention}"
        final_output = await message.reply_photo(
            photo="Utils/Telegram.JPEG",
...
async def start_live_stream(
    ...
    if not await join_live_stream(
        ...
        return await CallbackQuery.message.reply_text(
            f"Kesalahan Bergabung Obrolan Suara."
        )
